  ...
  return <main>
    ...
    <section className="card">
      {user ? <>
        ...
        <button className="secondary" onClick={listToWps}>查看 / 导出上架 WPS 表</button><button className="secondary" onClick={openLedger}>每日账单</button>
        {user.role === 'owner' && <><button className="secondary" onClick={loadTeam}>查看账号与工作状态</button>{team.length > 0 && <div className="grid">{team.map(x => <p key={x.id}>{x.username} · {roleName[x.role] || x.role} · 已上架 {x.orders} 单 {x.role !== 'owner' && <button className="text-button" onClick={() => removeMember(x.id)}>删除</button>}</p>)}</div>}</>}
      </> : <>
