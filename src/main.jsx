...
function App() {
  ...
  const generateAiReply = async () => {
    ...
    const r = await fetch('/api/ai/reply', { method:'POST', headers:{'content-type':'application/json'}, body:JSON.stringify({ message:aiMessage, quote:result ? quotePayload() : undefined }) })
  ...
  return <main>
    ...
    {user && <section className="card" hidden={activeSection !== 'ai'}><div className="section-title"><h2>DeepSeek AI 客服</h2><span className="notice">仅生成草稿，不会自动发送或上架</span></div><p className="notice">先在“报价工作台”完成计算后，这里会自动带入仅供客户查看的老板报价。打手价格、差值和利润不会发送给 DeepSeek。</p><label>客户原话<textarea value={aiMessage} onChange={e => setAiMessage(e.target.value)} placeholder="例如：这个号怎么报价？我现在可以上架吗？" /></label><button onClick={generateAiReply} disabled={aiLoading}>{aiLoading ? '正在生成…' : '生成客户回复草稿'}</button>{aiReply && <div className="ai-reply"><div className="section-title"><h3>发送前请核对</h3><button className="secondary" onClick={copyAiReply}>复制草稿</button></div><pre>{aiReply}</pre></div>}</section>}
