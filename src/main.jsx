...
const blankLedgerRow = () => ({ order_no: '', hafu_m: '', insurance_stamina: '', boss_final: '', worker_final: '', note: '' })
...
function App() {
  ...
  const updateLedgerRow = (index, key, value) => setLedgerRows(rows => rows.map((row, i) => i === index ? { ...row, [key]: value } : row))
  ...
  const loadLedger = async (date = ledgerDate) => { const r = await fetch(`/api/ledger?date=${encodeURIComponent(date)}`); const x = await responseJson(r); if (!r.ok) return setAppMsg(x.error || '无法读取每日账单。'); setLedgerRows(x.entries.length ? x.entries.map(({ order_no, hafu_m, insurance_stamina, boss_final, worker_final, note }) => ({ order_no, hafu_m, insurance_stamina, boss_final, worker_final, note })) : [blankLedgerRow()]); setHistoricalProfit(String(x.summary.historical_profit || '')); setLedgerSummary(x.summary) }
