...
function App() {
  ...
  const [lossDate, setLossDate] = useState(today()), [lossRows, setLossRows] = useState([]), [lossForm, setLossForm] = useState(blankLoss()), [lossSummary, setLossSummary] = useState(null), [lossSaving, setLossSaving] = useState(false)
  ...
  const updateMemberRole = async (id, role) => { const r = await fetch('/api/team/' + id, { method:'PUT', headers:{'content-type':'application/json'}, body:JSON.stringify({role}) }); const x = await responseJson(r); if (r.ok) { setAppMsg('职位已设置为' + roleName[x.role] + '。'); loadTeam() } else setAppMsg(x.error || '职位设置失败。') }
  ...
  const switchSection = section => {
    ...
    if (section === 'inventory') { loadListings(); if (user?.role === 'owner') loadTeam() }
  ...
  return <main>
    ...
    <section className="card" hidden={user && activeSection !== 'overview'}>
      {user ? <>
        ...
        <div className="totals"><div><span>待售上架单</span><strong>{listings.length} 单</strong></div><div><span>本月亏损</span><strong>{Number(ledgerSummary?.loss || 0).toFixed(2)} 元</strong></div><div><span>每月利润</span><strong>{Number(ledgerSummary?.net_profit || 0).toFixed(2)} 元</strong></div></div>
    ...
    {user && <section className="card" hidden={activeSection !== 'team' || user.role !== 'owner'}><div className="section-title"><h2>团队管理</h2><button className="secondary" onClick={loadTeam}>刷新</button></div><p className="notice">超级管理员可设置主管或员工；主管和员工只查看自己的库存、账单和工作记录。</p>{team.length > 0 ? <div className="team-list">{team.map(x => <div className="team-row" key={x.id}><span><b>{x.username}</b> · {roleName[x.role] || x.role}</span><span>已上架 {x.orders} 单</span>{x.role !== 'owner' && <select className="role-select" aria-label={x.username + '职位'} value={x.role} onChange={e => updateMemberRole(x.id,e.target.value)}><option value="supervisor">主管</option><option value="staff">员工</option></select>}{x.role !== 'owner' && <button className="text-button" onClick={() => removeMember(x.id)}>删除员工</button>}</div>)}</div> : <p>暂无团队账号。</p>}</section>}
