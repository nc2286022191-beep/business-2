...
function PublicStock() {
  ...
  useEffect(() => { document.title = '个人兴趣资料整理学习版'; fetch(`/api/public-inventory?code=${encodeURIComponent(code)}`).then(responseJson).then(result => result.error ? setError(result.error) : setData(result)).catch(() => setError('暂时无法读取练习资料，请稍后重试。')) }, [code])
  return <main className="public-stock"><header><h1>个人兴趣资料整理学习版</h1><p>公开练习链接 · 仅展示虚构兴趣资料</p></header>{error ? <section className="card"><h2>无法查看练习资料</h2><p className="notice">{error}</p></section> : !data ? <section className="card"><p>正在载入练习资料…</p></section> : <section className="card"><div className="section-title"><h2>{data.group} 组兴趣资料</h2><span className="notice">共 {data.listings.length} 条演示记录</span></div><div className="inventory-table-wrap"><table className="inventory-table"><thead><tr><th>练习编号</th><th>资源</th><th>状态</th><th>练习参数</th><th>区域</th><th>方式</th><th>演示数值</th><th>外观</th><th>AW</th><th>护甲</th><th>头盔</th><th>背包</th><th>时间</th><th>KD</th><th>段位</th></tr></thead><tbody>{data.listings.map(item => <tr key={item.order_no}><td>{item.order_no}</td><td>{item.hafu_m}m</td><td>{item.insurance_stamina}</td><td>{item.worker_ratio}</td><td>{item.region}</td><td>{item.login}</td><td>{item.worker_final}</td><td>{item.skins}</td><td>{item.aw}</td><td>{item.armor}</td><td>{item.redhead}</td><td>{item.redbag}</td><td>{item.online}</td><td>{item.kd}</td><td>{item.rank}</td></tr>)}</tbody></table></div><p className="notice">本页数据为系统生成的虚构练习资料，不对应任何真实信息。</p></section>}</main>
}
...
function App() {
  ...
  const loadShareLinks = async () => { const r = await fetch('/api/share-links'); const x = await responseJson(r); if (r.ok) setShareLinks(x.links || []); else setAppMsg(x.error || '无法读取打手链接。') }
  const shareUrl = code => `${location.origin}/stock/${code}`
  const copyShareLink = async code => { try { await navigator.clipboard.writeText(shareUrl(code)); setAppMsg('打手只读库存链接已复制。') } catch { setAppMsg('复制失败，请手动复制链接。') } }
  const createShareLink = async teamGroup => { const r = await fetch('/api/share-links', { method:'POST', headers:{'content-type':'application/json'}, body:JSON.stringify({team_group:teamGroup}) }); const x = await responseJson(r); if (r.ok) { setAppMsg(`${teamGroup} 组打手链接已生成。`); loadShareLinks() } else setAppMsg(x.error || '生成链接失败。') }
  const removeShareLink = async link => { if (!confirm(`关闭 ${link.team_group} 组的打手链接吗？关闭后打手将无法继续查看。`)) return; const r = await fetch(`/api/share-links/${link.id}`, {method:'DELETE'}); const x = await responseJson(r); if (r.ok) { setAppMsg('打手链接已关闭。'); loadShareLinks() } else setAppMsg(x.error || '关闭链接失败。') }
  ...
  const switchSection = section => {
    ...
    if (section === 'inventory') { loadListings(); loadShareLinks(); if (user?.role === 'owner') loadTeam() }
  ...
  {user && <section className="card" hidden={activeSection !== 'team' || user.role !== 'owner'}><div className="section-title"><h2>团队管理</h2><button className="secondary" onClick={loadTeam}>刷新</button></div><p className="notice">同一分组内的主管和员工共享上架库、利润表与亏损表；A 组是当前默认分组。修改分组后按 Tab 或点击其他位置保存。</p>{team.length > 0 ? <div className="team-list">{team.map(x => <div className="team-row" key={x.id}><span><b>{x.username}</b> · {roleName[x.role] || x.role}</span><span>{x.team_group} 组 · 已上架 {x.orders} 单</span>{x.role !== 'owner' && <select className="role-select" aria-label={x.username + '职位'} value={x.role} onChange={e => updateMember(x.id,{role:e.target.value})}><option value="supervisor">主管</option><option value="staff">员工</option></select>}<input className="group-input" aria-label={x.username + '分组'} defaultValue={x.team_group || 'A'} maxLength="20" onBlur={e => updateMember(x.id,{team_group:e.target.value})} />{x.role !== 'owner' && <button className="text-button" onClick={() => removeMember(x.id)}>删除员工</button>}</div>)}</div> : <p>暂无团队账号。</p>}</section>}
    ...
    {user && <section className="card" hidden={activeSection !== 'inventory'}>
      <div className="section-title"><h2>我的上架库</h2><button className="secondary" onClick={loadListings}>{listingsLoading ? '读取中…' : '刷新'}</button></div>
      ...
      <div className="section-title inventory-table-title"><h3>打手在线库存表</h3><span className="notice">只读 · 自动同步</span></div>
      <p className="notice">生成的链接格式为 <b>xiaokashanghang + 10 位数字</b>。打手打开后只能查看本组库存，不能编辑或进入后台。</p>
      <div className="share-link-list">{shareLinks.length ? shareLinks.map(link => <div className="share-link-row" key={link.team_group}><span><b>{link.team_group} 组</b><small>{link.share_code ? link.share_code : '尚未生成链接'}</small></span>{link.share_code ? <><button className="secondary" onClick={() => copyShareLink(link.share_code)}>复制链接</button><button className="text-button" onClick={() => removeShareLink(link)}>关闭链接</button></> : <button onClick={() => createShareLink(link.team_group)}>生成链接</button>}</div>) : <button onClick={() => createShareLink(user.team_group || 'A')}>为 {user.team_group || 'A'} 组生成打手链接</button>}</div>
    </section>}
