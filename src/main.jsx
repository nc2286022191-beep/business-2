...
function App() {
  ...
  return <main>
    ...
    {user && <section className="card" hidden={activeSection !== 'inventory'}><div className="section-title"><h2>我的上架库</h2><button className="secondary" onClick={loadListings}>{listingsLoading ? '读取中…' : '刷新'}</button></div>{listings.length ? <><div className="listing-list">{listings.map(item => <div className="listing-row" key={item.source_order_id}><div><b>{item.order_no}</b><span>{item.hafu_m}m · {item.insurance_stamina}</span><small>{item.note || '无备注'}</small></div><div><span>号主 {item.boss_final} · 打手 {item.worker_final}</span><strong>利润 {item.profit} 元</strong></div><div><button onClick={() => sellListing(item)}>已售出</button><button className="text-button" onClick={() => deleteListing(item)}>删除</button></div></div>)}</div><h3 className="inventory-table-title">上架库存表</h3><div className="inventory-table-wrap"><table className="inventory-table"><thead><tr><th>序号</th><th>纯币(m)</th><th>保险/体负</th><th>比例</th><th>大区</th><th>登录方式</th><th>全包价格</th><th>红皮 / 刀皮 / 砖皮</th><th>AW</th><th>红甲</th><th>红头</th><th>45包</th><th>在线时间</th><th>绝密 KD</th><th>段位</th></tr></thead><tbody>{listings.map(item => <tr key={`table-${item.source_order_id}`}><td>{item.order_no}</td><td>{item.hafu_m}</td><td>{item.insurance_stamina}</td><td>{item.boss_ratio || '—'}</td><td>{item.region || '—'}</td><td>{item.login || '—'}</td><td>{item.worker_final}</td><td>{[item.red_skin,item.knife_skin,item.brick_skin].filter(value => value && !['无','没有'].includes(value)).join(' / ') || '无'}</td><td>{item.aw}</td><td>{item.armor}</td><td>{item.redhead}</td><td>{item.redbag}</td><td>{item.online || '—'}</td><td>{item.kd || '—'}</td><td>{item.rank || '—'}</td></tr>)}</tbody></table></div></> : <p>{listingsLoading ? '正在读取上架库…' : '当前没有待售上架单。'}</p>}<p className="notice">库存表与待售上架单同步：售出或删除后会自动移除。全包价格为当前打手到手价。</p></section>}
