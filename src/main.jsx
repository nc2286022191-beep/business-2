...
function App() {
  ...
  const downloadInventory = () => {
    ...
    const rows = listings.map(item => [item.order_no,item.hafu_m,item.insurance_stamina,item.boss_ratio || '',item.region || '',item.login || '',item.worker_final,[item.red_skin,item.knife_skin,item.brick_skin].filter(value => value && !['无','没有'].includes(value)).join(' / ') || '无',item.aw,item.armor,item.redhead,item.redbag,item.online || '',item.kd || '',item.rank || ''])
    const csv = '\uFEFF' + [headers,...rows].map(row => row.map(escapeCsv).join(',')).join('\r\n')
    const url = URL.createObjectURL(new Blob([csv], { type:'text/csv;charset=utf-8' })), link = document.createElement('a')
    link.href = url; link.download = `上架库存表_${today()}.csv`; link.click(); URL.revokeObjectURL(url)
    setAppMsg(`库存表已下载，共 ${listings.length} 单。`)
  }
  ...
  return <main>
    ...
    {user && <section className="card" hidden={activeSection !== 'inventory'}><div className="section-title"><h2>我的上架库</h2><button className="secondary" onClick={loadListings}>{listingsLoading ? '读取中…' : '刷新'}</button></div>{listings.length ? <div className="listing-list">{listings.map(item => <div className="listing-row" key={item.source_order_id}><div><b>{item.order_no}</b><span>{item.hafu_m}m · {item.insurance_stamina}</span><small>{item.note || '无备注'}</small></div><div><span>号主 {item.boss_final} · 打手 {item.worker_final}</span><strong>利润 {item.profit} 元</strong></div><div><button onClick={() => sellListing(item)}>已售出</button><button className="text-button" onClick={() => deleteListing(item)}>删除</button></div></div>)}</div> : <p>{listingsLoading ? '正在读取上架库…' : '当前没有待售上架单。'}</p>}<div className="section-title inventory-table-title"><h3>上架库存表</h3><button className="secondary" onClick={downloadInventory}>下载库存表</button></div><div className="inventory-table-wrap"><table className="inventory-table"><thead><tr><th>序号</th><th>纯币(m)</th><th>保险/体负</th><th>比例</th><th>大区</th><th>登录方式</th><th>全包价格</th><th>红皮 / 刀皮 / 砖皮</th><th>AW</th><th>红甲</th><th>红头</th><th>45包</th><th>在线时间</th><th>绝密 KD</th><th>段位</th></tr></thead><tbody>{listings.length ? listings.map(item => <tr key={`table-${item.source_order_id}`}><td>{item.order_no}</td><td>{item.hafu_m}</td><td>{item.insurance_stamina}</td><td>{item.boss_ratio || '—'}</td><td>{item.region || '—'}</td><td>{item.login || '—'}</td><td>{item.worker_final}</td><td>{[item.red_skin,item.knife_skin,item.brick_skin].filter(value => value && !['无','没有'].includes(value)).join(' / ') || '无'}</td><td>{item.aw}</td><td>{item.armor}</td><td>{item.redhead}</td><td>{item.redbag}</td><td>{item.online || '—'}</td><td>{item.kd || '—'}</td><td>{item.rank || '—'}</td></tr>) : <tr><td className="inventory-empty" colSpan="15">暂无待售上架单</td></tr>}</tbody></table></div><p className="notice">库存表与待售上架单同步：售出或删除后会自动移除。全包价格为当前打手到手价。</p></section>}
