...
function PublicStock() {
  ...
  useEffect(() => { document.title = '个人兴趣资料整理学习版'; fetch(`/api/public-inventory?code=${encodeURIComponent(code)}`).then(responseJson).then(result => result.error ? setError(result.error) : setData(result)).catch(() => setError('暂时无法读取资料，请稍后重试。')) }, [code])
  return <main className="public-stock"><header><h1>个人兴趣资料整理学习版</h1></header><p className="public-disclaimer">本链接仅供参考，不具备任何意义，不允许有任何商业用途。</p>{error ? <section className="card"><h2>无法查看资料</h2><p className="notice">{error}</p></section> : !data ? <section className="card"><p>正在载入资料…</p></section> : <section className="card"><div className="section-title"><h2>{data.group} 组资料表</h2></div><div className="inventory-table-wrap"><table className="inventory-table"><thead><tr><th>序号</th><th>纯币(m)</th><th>保险/体负</th><th>打手比例</th><th>大区</th><th>登录方式</th><th>全包价格</th><th>红皮 / 刀皮 / 砖皮</th><th>AW</th><th>红甲</th><th>红头</th><th>45包</th><th>在线时间</th><th>绝密 KD</th><th>段位</th></tr></thead><tbody>{data.listings.length ? data.listings.map(item => <tr key={item.order_no}><td>{item.order_no}</td><td>{item.hafu_m}</td><td>{item.insurance_stamina}</td><td>{item.worker_ratio || '—'}</td><td>{item.region || '—'}</td><td>{item.login || '—'}</td><td>{item.worker_final}</td><td>{item.skins}</td><td>{item.aw}</td><td>{item.armor}</td><td>{item.redhead}</td><td>{item.redbag}</td><td>{item.online || '—'}</td><td>{item.kd || '—'}</td><td>{item.rank || '—'}</td></tr>) : <tr><td className="inventory-empty" colSpan="15">暂无资料</td></tr>}</tbody></table></div></section>}</main>
}
...
function App() {
  ...
  {user && <section className="card" hidden={activeSection !== 'team' || user.role !== 'owner'}><div className="section-title"><h2>团队管理</h2><button className="secondary" onClick={loadTeam}>刷新</button></div><p className="notice">同一分组内的主管和员工共享上架库、利润表与亏损表；A 组是当前默认分组。修改分组后按 Tab 或点击其他位置保存。</p>{team.length > 0 ? <div className="team-list">{team.map(x => <div className="team-row" key={x.id}><span><b>{x.username}</b> · {roleName[x.role] || x.role}</span><span>{x.team_group} 组 · 已上架 {x.orders} 单</span>{x.role !== 'owner' && <select className="role-select" aria-label={x.username + '职位'} value={x.role} onChange={e => updateMember(x.id,{role:e.target.value})}><option value="supervisor">主管</option><option value="staff">员工</option></select>}<input className="group-input" aria-label={x.username + '分组'} defaultValue={x.team_group || 'A'} maxLength="20" onBlur={e => updateMember(x.id,{team_group:e.target.value})} />{x.role !== 'owner' && <button className="text-button" onClick={() => removeMember(x.id)}>删除员工</button>}</div>)}</div> : <p>暂无团队账号。</p>}</section>}
    ...
    {user && <section className="card" hidden={activeSection !== 'inventory'}>
      ...
      <div className="section-title inventory-table-title"><h3>打手在线库存表</h3></div>
