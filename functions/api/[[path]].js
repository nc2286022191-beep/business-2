...
async function actor(request, env) {
  ...
  return env.DB.prepare(`SELECT u.id,u.username,u.role,u.team_group FROM sessions s JOIN users u ON u.id=s.user_id WHERE s.token_hash=? AND s.expires_at>? AND u.disabled_at IS NULL`).bind(tokenHash, now()).first()
}
...
function calculate(p) {
  ...
  if (workerRatio >= bossRatio) throw new Error('打手比例必须小于号主比例。')
...
async function ensureTeamSchema(env) {
  ...
  await add('users','disabled_at','TEXT')
...
async function ensureShareSchema(env) {
  ...
  await env.DB.prepare('CREATE TABLE IF NOT EXISTS member_inventory_share_links (id TEXT PRIMARY KEY, user_id TEXT NOT NULL UNIQUE, share_code TEXT NOT NULL UNIQUE, active INTEGER NOT NULL DEFAULT 1, created_at TEXT NOT NULL, updated_at TEXT NOT NULL, FOREIGN KEY(user_id) REFERENCES users(id))').run()
}
const shareCode = () => `simlearn${String(crypto.getRandomValues(new Uint32Array(1))[0]).padStart(10,'0')}`
function publicListing(order) {
  const p=JSON.parse(order.payload_json), c=JSON.parse(order.calculation_json)
  return {order_no:order.order_no,hafu_m:ledgerNumber(p.hafu_m),insurance_stamina:`${p.insurance||''}格/${p.stamina||''}`,worker_ratio:p.worker_ratio||'',region:p.region||'',login:p.login||'',worker_final:ledgerNumber(c.worker?.final),skins:[p.red_skin,p.knife_skin,p.brick_skin].filter(x=>x&&!['无','没有'].includes(x)).join(' / ')||'无',aw:ledgerNumber(p.aw),armor:ledgerNumber(p.armor),redhead:ledgerNumber(p.redhead),redbag:ledgerNumber(p.redbag),online:p.online||'',kd:p.kd||'',rank:p.rank||''}
}
...
export async function onRequest({ request, env }) {
  ...
  if (request.method === 'GET' && path === '/security-question') { const username=validText(url.searchParams.get('username'),80); if(!username)return fail('请先填写账号。'); const user=await env.DB.prepare('SELECT security_question FROM users WHERE username=? AND disabled_at IS NULL').bind(username).first(); return user ? json({question:user.security_question}) : fail('账号不存在。',404) }
  if (request.method === 'GET' && path === '/me') { const user=await actor(request,env); return user ? json({user}) : fail('请先登录',401) }
  if (request.method === 'GET' && path === '/public-inventory') { const code=validText(url.searchParams.get('code'),80); if(!/^(?:xiaokashanghang|simlearn)\d{10}$/.test(code))return fail('链接无效。',404); await ensureShareSchema(env); const memberLink=await env.DB.prepare('SELECT user_id FROM member_inventory_share_links WHERE share_code=? AND active=1').bind(code).first(); if(memberLink){ const rows=await env.DB.prepare("SELECT * FROM orders WHERE operator_id=? AND stage='已上架' AND deleted_at IS NULL ORDER BY json_extract(payload_json,'$.hafu_m') ASC").bind(memberLink.user_id).all(); return json({scope:'personal',listings:(rows.results||[]).map(publicListing),synced_at:now()}) } const link=await env.DB.prepare('SELECT team_group FROM inventory_share_links WHERE share_code=? AND active=1').bind(code).first(); if(!link)return fail('该链接已关闭或不存在。',404); const rows=await env.DB.prepare("SELECT * FROM orders WHERE team_group=? AND stage='已上架' AND deleted_at IS NULL ORDER BY json_extract(payload_json,'$.hafu_m') ASC").bind(link.team_group).all(); return json({scope:'group',group:link.team_group,listings:(rows.results||[]).map(publicListing),synced_at:now()}) }
  ...
  if (request.method === 'DELETE' && /^\/share-links\/[^/]+$/.test(path)) { const user=await actor(request,env); if(!user)return fail('请先登录',401); if(user.role!=='owner')return fail('仅管理员可管理资料表链接。',403); await ensureShareSchema(env); const record=await env.DB.prepare('SELECT id FROM inventory_share_links WHERE id=?').bind(path.slice('/share-links/'.length)).first(); if(!record)return fail('链接不存在。',404); await env.DB.prepare('DELETE FROM inventory_share_links WHERE id=?').bind(record.id).run(); return json({deleted:true}) }
  if (request.method === 'GET' && path === '/wps.csv') { const user=await actor(request,env); if(!user)return fail('请先登录',401); const scope=groupKey(user), query=scope?"SELECT o.order_no,o.stage,o.payload_json,o.calculation_json,u.username FROM orders o JOIN users u ON u.id=o.operator_id WHERE o.team_group=? AND o.deleted_at IS NULL AND o.stage='已上架' ORDER BY json_extract(o.payload_json,'$.hafu_m') ASC":"SELECT o.order_no,o.stage,o.payload_json,o.calculation_json,u.username FROM orders o JOIN users u ON u.id=o.operator_id WHERE o.deleted_at IS NULL AND o.stage='已上架' ORDER BY json_extract(o.payload_json,'$.hafu_m') ASC"; const rows=await env.DB.prepare(query).bind(...(scope?[scope]:[])).all(); const esc=x=>'"'+String(x??'').replaceAll('"','""')+'"'; const head=['编号','纯币(m)','保险体负','打手比例','大区','登录方式','老板到手','打手到手','差值','皮肤','计费项','操作人']; const csv='\uFEFF'+[head,...(rows.results||[]).map(r=>{const p=JSON.parse(r.payload_json),c=JSON.parse(r.calculation_json);return[r.order_no,p.hafu_m,`${p.insurance||''}格/${p.stamina||''}`,p.worker_ratio||'',p.region||'',p.login||'',c.boss?.final||'',c.worker?.final||'',c.difference||'',c.skins||'',c.item_note||'',r.username]})].map(a=>a.map(esc).join(',')).join('\r\n'); return new Response(csv,{headers:{'content-type':'text/csv; charset=utf-8','content-disposition':'attachment; filename="wps-listing.csv"'}}) }
  if (request.method !== 'POST') return fail('未找到接口',404)
  let body={}; try { body=await request.json() } catch { return fail('请求格式错误') }
  if (path === '/my-share-link') { const user=await actor(request,env); if(!user)return fail('请先登录',401); await ensureShareSchema(env); const existing=await env.DB.prepare('SELECT id,share_code,active,updated_at FROM member_inventory_share_links WHERE user_id=?').bind(user.id).first(); if(existing)return json({link:existing}); const record={id:id(),user_id:user.id,share_code:shareCode(),active:1,updated_at:now()}; await env.DB.prepare('INSERT INTO member_inventory_share_links(id,user_id,share_code,active,created_at,updated_at) VALUES(?,?,?,?,?,?)').bind(record.id,record.user_id,record.share_code,record.active,record.updated_at,record.updated_at).run(); return json({link:record},201) }
  if (path === '/register') {
  ...
  if (path === '/login') {
    const username=validText(body.username,80), user=await env.DB.prepare('SELECT * FROM users WHERE username=? AND disabled_at IS NULL').bind(username).first()
  ...
  if (path === '/password-reset') {
    ...
    const user=await env.DB.prepare('SELECT * FROM users WHERE username=? AND disabled_at IS NULL').bind(username).first()
  ...
  if (path === '/ai/reply') return fail('个人兴趣学习版已关闭对外回复功能。',403)
  if (path === '/share-links') { if(user.role!=='owner')return fail('仅管理员可创建资料表链接。',403); await ensureShareSchema(env); const teamGroup=groupName(body.team_group||user.team_group), existing=await env.DB.prepare('SELECT id,team_group,share_code,active,updated_at FROM inventory_share_links WHERE team_group=?').bind(teamGroup).first(); if(existing)return json({link:existing}); const record={id:id(),team_group:teamGroup,share_code:shareCode(),active:1,updated_at:now()}; await env.DB.prepare('INSERT INTO inventory_share_links(id,team_group,share_code,active,created_at,updated_at) VALUES(?,?,?,?,?,?)').bind(record.id,record.team_group,record.share_code,record.active,record.updated_at,record.updated_at).run(); return json({link:record},201) }
  if (path === '/ai/reply') {
