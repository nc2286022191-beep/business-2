...
async function lossTotal(env, userId, month) {
  await ensureLossSchema(env)
  const total=await env.DB.prepare("SELECT COALESCE(SUM(loss),0) AS loss FROM personal_loss_records WHERE user_id=? AND substr(loss_date,1,7)=?").bind(userId,month).first()
...
async function ledgerSummary(env, userId, date) {
  ...
  const historical = ledgerNumber(baseline?.historical_profit), profit = ledgerNumber(totals?.profit), loss=await lossTotal(env,userId,month)
...
export async function onRequest({ request, env }) {
  ...
  if (request.method === 'GET' && path === '/losses') { const user=await actor(request,env); if(!user)return fail('请先登录',401); const date=ledgerDate(url.searchParams.get('date')) || chinaDate(); await ensureLossSchema(env); const rows=await env.DB.prepare('SELECT id,loss_date,order_no,aw,six_head,six_armor,bag45,discounted_total,original_total,loss,note FROM personal_loss_records WHERE user_id=? AND loss_date=? ORDER BY created_at DESC').bind(user.id,date).all(); return json({date,records:rows.results||[],month_loss:await lossTotal(env,user.id,date.slice(0,7)),summary:await ledgerSummary(env,user.id,date)}) }
  ...
  if (request.method === 'PUT' && /^\/team\/[^/]+$/.test(path)) { const user=await actor(request,env); if(!user)return fail('请先登录',401); if(user.role!=='owner')return fail('仅总设计师可设置职位',403); let body={}; try { body=await request.json() } catch { return fail('请求格式错误。') } const target=path.slice('/team/'.length), role=validText(body.role,30); if(!['supervisor','staff'].includes(role))return fail('职位只能设置为主管或员工。'); const found=await env.DB.prepare('SELECT id,role FROM users WHERE id=?').bind(target).first(); if(!found)return fail('员工不存在。',404); if(found.role==='owner')return fail('不能修改超级管理员职位。'); await env.DB.prepare('UPDATE users SET role=? WHERE id=?').bind(role,target).run(); return json({updated:true,role}) }
  ...
  if (request.method === 'DELETE' && /^\/losses\/[^/]+$/.test(path)) { const user=await actor(request,env); if(!user)return fail('请先登录',401); await ensureLossSchema(env); const record=await env.DB.prepare('SELECT id FROM personal_loss_records WHERE id=? AND user_id=?').bind(path.slice('/losses/'.length),user.id).first(); if(!record)return fail('亏损记录不存在或无权限。',404); await env.DB.prepare('DELETE FROM personal_loss_records WHERE id=?').bind(record.id).run(); return json({deleted:true}) }
  ...
  if (path === '/losses') { try { const lossDate=ledgerDate(body.loss_date); const orderNo=validText(body.order_no,80); if(!lossDate)return fail('请选择正确的亏损日期。'); if(!orderNo)return fail('请填写群名中的编号。'); await ensureLossSchema(env); const amount=lossAmounts(body), note=validText(body.note,500); await env.DB.prepare('INSERT INTO personal_loss_records(id,user_id,loss_date,order_no,aw,six_head,six_armor,bag45,discounted_total,original_total,loss,note,created_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)').bind(id(),user.id,lossDate,orderNo,amount.aw,amount.six_head,amount.six_armor,amount.bag45,amount.discounted_total,amount.original_total,amount.loss,note,now()).run(); return json({saved:true,record:{loss_date:lossDate,order_no:orderNo,...amount,note},summary:await ledgerSummary(env,user.id,lossDate)}) } catch(e){return fail(e.message)} }
