...
async function ensureLedgerSchema(env) {
  await env.DB.batch([
    env.DB.prepare('CREATE TABLE IF NOT EXISTS daily_ledgers (id TEXT PRIMARY KEY, ledger_date TEXT UNIQUE NOT NULL, updated_by TEXT NOT NULL, updated_at TEXT NOT NULL)'),
    env.DB.prepare('CREATE TABLE IF NOT EXISTS ledger_entries (id TEXT PRIMARY KEY, ledger_id TEXT NOT NULL, row_order INTEGER NOT NULL, order_no TEXT NOT NULL, hafu_m REAL NOT NULL, insurance_stamina TEXT NOT NULL, boss_final REAL NOT NULL, worker_final REAL NOT NULL, profit REAL NOT NULL, note TEXT NOT NULL, FOREIGN KEY(ledger_id) REFERENCES daily_ledgers(id))'),
    env.DB.prepare('CREATE TABLE IF NOT EXISTS monthly_baselines (month_key TEXT PRIMARY KEY, historical_profit REAL NOT NULL DEFAULT 0, updated_at TEXT NOT NULL)')
  ])
}
async function ledgerSummary(env, date) {
  const month = date.slice(0, 7)
  const baseline = await env.DB.prepare('SELECT historical_profit FROM monthly_baselines WHERE month_key=?').bind(month).first()
  const totals = await env.DB.prepare("SELECT COUNT(e.id) AS orders, COUNT(DISTINCT l.ledger_date) AS days, COALESCE(SUM(e.hafu_m),0) AS hafu_m, COALESCE(SUM(e.boss_final),0) AS boss_total, COALESCE(SUM(e.worker_final),0) AS worker_total, COALESCE(SUM(e.profit),0) AS profit FROM daily_ledgers l LEFT JOIN ledger_entries e ON e.ledger_id=l.id WHERE substr(l.ledger_date,1,7)=?").bind(month).first()
...
export async function onRequest({ request, env }) {
  ...
  if (request.method === 'GET' && path === '/ledger') { const user=await actor(request,env); if(!user)return fail('请先登录',401); if(user.role!=='owner')return fail('仅超级管理员可查看每日账单',403); const date=ledgerDate(url.searchParams.get('date')); if(!date)return fail('日期格式不正确。'); await ensureLedgerSchema(env); const ledger=await env.DB.prepare('SELECT * FROM daily_ledgers WHERE ledger_date=?').bind(date).first(); const entries=ledger ? await env.DB.prepare('SELECT id,row_order,order_no,hafu_m,insurance_stamina,boss_final,worker_final,profit,note FROM ledger_entries WHERE ledger_id=? ORDER BY row_order').bind(ledger.id).all() : {results:[]}; return json({date,entries:entries.results||[],summary:await ledgerSummary(env,date)}) }
  ...
  if (path === '/ledger') { try { if(user.role!=='owner')return fail('仅超级管理员可保存每日账单',403); const date=ledgerDate(body.date); if(!date)return fail('请选择正确日期。'); const rows=Array.isArray(body.entries) ? body.entries.slice(0,200) : []; const entries=rows.map((row,index)=>({row_order:index+1,order_no:validText(row.order_no,80),hafu_m:ledgerNumber(row.hafu_m),insurance_stamina:validText(row.insurance_stamina,80),boss_final:ledgerNumber(row.boss_final),worker_final:ledgerNumber(row.worker_final),note:validText(row.note,500)})).filter(row=>row.order_no || row.hafu_m || row.boss_final || row.worker_final || row.note).map(row=>({...row,profit:row.worker_final-row.boss_final})); await ensureLedgerSchema(env); const existing=await env.DB.prepare('SELECT id FROM daily_ledgers WHERE ledger_date=?').bind(date).first(); const ledgerId=existing?.id || id(); const month=date.slice(0,7), baseline=ledgerNumber(body.historical_profit); const statements=[env.DB.prepare('INSERT INTO daily_ledgers(id,ledger_date,updated_by,updated_at) VALUES(?,?,?,?) ON CONFLICT(ledger_date) DO UPDATE SET updated_by=excluded.updated_by,updated_at=excluded.updated_at').bind(ledgerId,date,user.id,now()),env.DB.prepare('DELETE FROM ledger_entries WHERE ledger_id=?').bind(ledgerId),env.DB.prepare('INSERT INTO monthly_baselines(month_key,historical_profit,updated_at) VALUES(?,?,?) ON CONFLICT(month_key) DO UPDATE SET historical_profit=excluded.historical_profit,updated_at=excluded.updated_at').bind(month,baseline,now()),...entries.map(row=>env.DB.prepare('INSERT INTO ledger_entries(id,ledger_id,row_order,order_no,hafu_m,insurance_stamina,boss_final,worker_final,profit,note) VALUES(?,?,?,?,?,?,?,?,?,?)').bind(id(),ledgerId,row.row_order,row.order_no,row.hafu_m,row.insurance_stamina,row.boss_final,row.worker_final,row.profit,row.note))]; await env.DB.batch(statements); return json({saved:true,date,entries,summary:await ledgerSummary(env,date)}) } catch(e){return fail(e.message)} }
