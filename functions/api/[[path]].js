...
export async function onRequest({ request, env }) {
  ...
  if (request.method === 'GET' && path === '/ledger') { const user=await actor(request,env); if(!user)return fail('请先登录',401); const date=ledgerDate(url.searchParams.get('date')); if(!date)return fail('日期格式不正确。'); await ensureLedgerSchema(env); const ledger=await env.DB.prepare('SELECT * FROM personal_daily_ledgers WHERE user_id=? AND ledger_date=?').bind(user.id,date).first(); const entries=ledger ? await env.DB.prepare('SELECT id,row_order,order_no,hafu_m,insurance_stamina,boss_final,worker_final,profit,note FROM personal_ledger_entries WHERE ledger_id=? ORDER BY row_order').bind(ledger.id).all() : {results:[]}; return json({date,entries:entries.results||[],summary:await ledgerSummary(env,user.id,date)}) }
  ...
  if (path === '/ledger') { try { const date=ledgerDate(body.date); if(!date)return fail('请选择正确日期。'); const rows=Array.isArray(body.entries) ? body.entries.slice(0,200) : []; const entries=rows.map((row,index)=>({row_order:index+1,order_no:validText(row.order_no,80),hafu_m:ledgerNumber(row.hafu_m),insurance_stamina:validText(row.insurance_stamina,80),boss_final:ledgerNumber(row.boss_final),worker_final:ledgerNumber(row.worker_final),note:validText(row.note,500)})).filter(row=>row.order_no || row.hafu_m || row.boss_final || row.worker_final || row.note).map(row=>({...row,profit:row.worker_final-row.boss_final})); await ensureLedgerSchema(env); const existing=await env.DB.prepare('SELECT id FROM personal_daily_ledgers WHERE user_id=? AND ledger_date=?').bind(user.id,date).first(); const ledgerId=existing?.id || id(); const month=date.slice(0,7), baseline=ledgerNumber(body.historical_profit); const statements=[env.DB.prepare('INSERT INTO personal_daily_ledgers(id,ledger_date,user_id,updated_at) VALUES(?,?,?,?) ON CONFLICT(user_id,ledger_date) DO UPDATE SET updated_at=excluded.updated_at').bind(ledgerId,date,user.id,now()),env.DB.prepare('DELETE FROM personal_ledger_entries WHERE ledger_id=?').bind(ledgerId),env.DB.prepare('INSERT INTO personal_monthly_baselines(user_id,month_key,historical_profit,updated_at) VALUES(?,?,?,?) ON CONFLICT(user_id,month_key) DO UPDATE SET historical_profit=excluded.historical_profit,updated_at=excluded.updated_at').bind(user.id,month,baseline,now()),...entries.map(row=>env.DB.prepare('INSERT INTO personal_ledger_entries(id,ledger_id,row_order,order_no,hafu_m,insurance_stamina,boss_final,worker_final,profit,note) VALUES(?,?,?,?,?,?,?,?,?,?)').bind(id(),ledgerId,row.row_order,row.order_no,row.hafu_m,row.insurance_stamina,row.boss_final,row.worker_final,row.profit,row.note))]; await env.DB.batch(statements); return json({saved:true,date,entries,summary:await ledgerSummary(env,user.id,date)}) } catch(e){return fail(e.message)} }
