...
const simulationListings = group => [
  {order_no:`SIM-${group}-01`,hafu_m:120,insurance_stamina:'9格/满',worker_ratio:'学习参数',region:'模拟区域',login:'模拟方式',worker_final:260,skins:'演示外观',aw:36,armor:2,redhead:1,redbag:1,online:'模拟时间',kd:'—',rank:'练习段位'},
  {order_no:`SIM-${group}-02`,hafu_m:180,insurance_stamina:'6格/满',worker_ratio:'学习参数',region:'模拟区域',login:'模拟方式',worker_final:340,skins:'演示外观',aw:52,armor:3,redhead:2,redbag:2,online:'模拟时间',kd:'—',rank:'练习段位'}
]
...
export async function onRequest({ request, env }) {
  ...
  if (request.method === 'GET' && path === '/public-inventory') { const code=validText(url.searchParams.get('code'),80); if(! /^(?:xiaokashanghang|simlearn)\d{10}$/.test(code))return fail('模拟链接无效。',404); await ensureShareSchema(env); const link=await env.DB.prepare('SELECT team_group FROM inventory_share_links WHERE share_code=? AND active=1').bind(code).first(); if(!link)return fail('该模拟链接已关闭或不存在。',404); return json({group:link.team_group,listings:simulationListings(link.team_group),simulation:true}) }
  ...
  if (path === '/register') return fail('模拟经营学习版暂不开放自助注册。',403)
  if (path === '/register') {
  ...
  if (path === '/ai/reply') return fail('模拟经营学习版已关闭对外回复功能。',403)
